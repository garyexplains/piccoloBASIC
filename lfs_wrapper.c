...
#endif
static_assert(PICCOLOBASIC_HW_FLASH_STORAGE_BYTES % 4096 == 0,
              "Flash storage size must be a multiple of 4K");

#ifndef PICCOLOBASIC_HW_FLASH_STORAGE_BASE
#define PICCOLOBASIC_HW_FLASH_STORAGE_BASE                                     \
  (PICO_FLASH_SIZE_BYTES - PICCOLOBASIC_HW_FLASH_STORAGE_BYTES)
#endif

static_assert(PICCOLOBASIC_HW_FLASH_STORAGE_BYTES <= PICO_FLASH_SIZE_BYTES,
              "PICCOLOBASIC_HW_FLASH_STORAGE_BYTES too big");
static_assert(PICCOLOBASIC_HW_FLASH_STORAGE_BASE +
                      PICCOLOBASIC_HW_FLASH_STORAGE_BYTES <=
...
int pico_lfsflash_read(const struct lfs_config *c, lfs_block_t block,
...
int pico_lfsflash_prog(const struct lfs_config *c, lfs_block_t block,
...
int pico_lfsflash_sync(const struct lfs_config *c) { return 0; }
...
int lfswrapper_file_close() { return lfs_file_close(&lfs, &current_lfs_file); }
...
int lfswrapper_delete_file(char *path) { return lfs_remove(&lfs, path); }
